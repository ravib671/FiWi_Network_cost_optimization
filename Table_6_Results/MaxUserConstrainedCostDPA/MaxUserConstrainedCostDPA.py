...
This  algorithm based dynamic programming  finds percentage of cost of O for which P includes 100% users 
for each network size 


Parameters:
   network size : 200,250,300,350,400
   Percentage of cost of O considered : 60-100%  
...
def save_results(no_of_users_P, sel_onu_P,cmax, cmax_val, onu_users_count_lst, normlzd_onu_cost_lst, m):
        ...
        f.write("Number of onu in P = " + str(len(sel_onu_P)) + "\n")
        # % user inclusion   
...
def MaxUserConstrainedCostDPA(nw_file, m, cmax_val):
        ...
        no_of_users_P, sel_onu_P = MaximizeUserDPA(onu_users_count_lst, normlzd_onu_cost_lst,int(cmax))
