...
Input : ONU size and n/w size
Output : Minimized deployment with all users
...
def build_node_cost_lst(nw_file_path):
    ...
    #Read associated users in each ONU
...
def save_results(normlzd_onu_cost_lst, selected_ONU_pos, Pcost, m):
        f = open("Result_table2.txt","a")

        f.write("Number of ONU in O=" + str(m) + "\n\n")

        f.write("Number of users in ONU O= 300" + "\n")
...
#Start here
def MinCostAllUser(m):
        nw_file_path = "../data" + "/network_size_300/" + str(m) 
        ...
        save_results(normlzd_onu_cost_lst, selected_ONU_pos, Pcost,m)
